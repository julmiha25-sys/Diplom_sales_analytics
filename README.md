...
│
├── 🔧 Конфигурация
│   ├── .env          (текущие настройки – скрытая информация)
│   └── .env.sample   (шаблон для загрузки в GitHub)
│
├── 🐍 Виртуальное окружение
│   └── venv/
│
├── 📄 Скрипты Python
│   ├── pgdb.py             (работа с PostgreSQL, синглтон)
│   ├── run-all.py          (единоразовый запуск для получения всех исторических данных)
│   └── run-yesterday.py    (ежедневный запуск в crontab для загрузки данных за вчерашний день)
│
├── 📦 Данные (CSV-бэкапы продаж для ежедневного анализа, хранится 3 последние копии)
│   ├── sales_backup_2026-04-28.csv
│   ├── sales_backup_2026-04-29.csv
│   └── sales_backup_2026-04-30.csv
│
├── 🗂️ Логи / кэш Python (разовой загрузки, ежедневных загрузок, crontab)
│   ├── logs/
│   │   ├── etl_20260428_120000.log
│   │   ├── daily_etl_20260429_120000.log
│   │   └── daily_etl_20260430_120000.log
│   └── __pycache__/
└── 🖥️ VM — виртуальная машина
